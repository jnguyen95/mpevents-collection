...
char *lucky_coin_not_full_inventory_msg =
    "It\x5Cs a Lucky Coin\xC2 It must be your lucky day\xC2"
    "\x0A"
    "You picked up the Lucky Coin\x85"
    "\xFF";

char *lucky_coin_full_inventory_msg =
    "It\x5Cs a bison dollar\x85"
    "\x0A"
    "You leave the bison dollar on the ground\x85"
    "\xFF";
...
void main() {
    ...
    ShowMessage(-1, lucky_coin_msg, 0, 0, 0, 0, 0);
    cleanupMessage();
    s32 open_index = PlayerHasEmptyItemSlot(cur_player_index);
    if (open_index != -1) {
        struct player *player = GetPlayerStruct(cur_player_index);
        ShowMessage(-1, lucky_coin_not_full_inventory_msg, 0, 0, 0, 0, 0);
        cleanupMessage();
        player->items[open_index] = ITEM_LUCKY_COIN;
    }
    else {
        ShowMessage(-1, lucky_coin_full_inventory_msg, 0, 0, 0, 0, 0);
        cleanupMessage();
    }
}