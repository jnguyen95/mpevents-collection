...
char *barter_box_not_full_inventory_msg =
    "The box speaks and says "
    "\xC0" // "
    "You must get the lamps at all cost\xC2"
    "\x0A" // newline
    "Take my prized box\xC2"
    "\xC1" // "
    "\xFF";

char *barter_box_full_inventory_msg =
    "You go near it only to realize it\x5Cs a mimic\xC2"
    "\x0A"
    "You run away\x82 but kept your items safe\x85"
    "\xFF";
...
void main() {
    ...
    ShowMessage(-1, barter_box_msg, 0, 0, 0, 0, 0);
    cleanupMessage();
    s32 open_index = PlayerHasEmptyItemSlot(cur_player_index);
    if (open_index != -1) {
        struct player *player = GetPlayerStruct(cur_player_index);
        ShowMessage(-1, barter_box_not_full_inventory_msg, 0, 0, 0, 0, 0);
        cleanupMessage();
        player->items[open_index] = ITEM_BARTER_BOX;
    }
    else {
        ShowMessage(-1, barter_box_full_inventory_msg, 0, 0, 0, 0, 0);
        cleanupMessage();
    }
}